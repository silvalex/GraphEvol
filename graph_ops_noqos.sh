...
cp $DIR_PROGRAM"graph-evol-noqos.params" .
cp $ECJ_JAR .
cp $1/* . # Copy datasets
...
java -cp program.jar:ecj.23.jar:. ec.Evolve -file $3 -p seed.0=$seed -p stat.file=\$$result -p stat.histogram=\$$hist_result -p running-owls=$4 -p find-concepts=$5 -p composition-task=$6 -p composition-taxonomy=$7 -p composition-services=$8
